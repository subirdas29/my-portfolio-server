...
export const extractTechFromHistory = (history: { role: string; content: string }[]): string | null => {
  if (!history || history.length === 0) return null;
  for (let i = history.length - 1; i >= 0; i--) {
    const msg = history[i].content.toLowerCase();
    for (const tech of TECH_KEYWORDS) {
      if (msg.includes(tech)) return tech;
    }
  }
  return null;
};

export const isSkillFromHistory = (history: { role: string; content: string }[]): boolean => {
  if (!history || history.length === 0) return false;
  for (let i = history.length - 1; i >= 0; i--) {
    const msg = history[i].content.toLowerCase();
    if (msg.includes('skill') || msg.includes('technology') || msg.includes('tech stack')) return true;
  }
  return false;
};

export const isBlogFromHistory = (history: { role: string; content: string }[]): boolean => {
  if (!history || history.length === 0) return false;
  for (let i = history.length - 1; i >= 0; i--) {
    const msg = history[i].content.toLowerCase();
    if (msg.includes('blog') || msg.includes('article') || msg.includes('writing')) return true;
  }
  return false;
};
