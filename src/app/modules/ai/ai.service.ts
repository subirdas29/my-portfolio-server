...
const getPinecone = () => {
  if (!pineconeClient) {
    pineconeClient = new Pinecone({ apiKey: process.env.PINECONE_API_KEY as string });
  }
...
const PROJECT_DETAILS_URL = (slug: string) => `${SITE_URL}/all-projects/projectDetails/${slug}`;
const ALL_PROJECTS_URL = `${SITE_URL}/all-projects`;
const FALLBACK_MESSAGE = getFallbackMessage(SITE_URL);

// ==================== Context Builders ====================
const createProjectContext = (doc: any) =>
  [
    `PROJECT: ${doc.title}`,
...
const createProjectMetadata = (doc: any) => ({
  title: doc.title,
...
const createBlogContext = (doc: any) =>
  [
    `BLOG: ${doc.title}`,
    `Summary: ${doc.summary || ''}`,
...
const createBlogMetadata = (doc: any) => ({
  title: doc.title,
...
const createSkillContext = (doc: any) => {
  const technologies = doc.logo.filter((item: string) => !item.startsWith('http'));
  return `SKILL CATEGORY: ${doc.title}\nTechnologies: ${technologies.join(', ')}`;
};

const createSkillMetadata = (doc: any) => {
  const technologies = doc.logo.filter((item: string) => !item.startsWith('http'));
  return {
    title: doc.title,
...
// ==================== Upsert Functions ====================
export const upsertProjectToAI = async (doc: any) => {
  const text = createProjectContext(doc);
  const embedding = await getEmbedding(text);
  const index = getPinecone().Index(process.env.PINECONE_INDEX as string);
...
export const upsertBlogToAI = async (doc: any) => {
...
export const upsertSkillToAI = async (doc: any) => {
...
export const upsertExperienceToAI = async () => {
  const index = getPinecone().Index(process.env.PINECONE_INDEX as string);
...
export const upsertEducationToAI = async () => {
  const index = getPinecone().Index(process.env.PINECONE_INDEX as string);
  const records = await Promise.all(
...
export const upsertAboutMeToAI = async () => {
  ...
  await index.namespace(NAMESPACE).upsert([
    {
      id: 'aboutme_0',
      values: embedding,
      metadata: { title: 'About Subir Das', content: ABOUT_ME_DATA, type: 'aboutme' },
    },
  ]);
};

export const upsertAllStaticDataToAI = async () => {
  console.log('Upserting experience, education, about me to Pinecone...');
  await Promise.all([upsertExperienceToAI(), upsertEducationToAI(), upsertAboutMeToAI()]);
  console.log('All static data upserted successfully.');
};

export const deleteFromAI = async (id: string) => {
  const index = getPinecone().Index(process.env.PINECONE_INDEX as string);
  await index.namespace(NAMESPACE).deleteOne({ id } as any);
};

export const searchPinecone = async (query: string, topK = DEFAULT_TOP_K) => {
...
export const searchProjectsByTechnology = async (techName: string) => {
  const regex = new RegExp(techName, 'i');
  const projects = await Project.find({ technologies: { $regex: regex } })
...
export const getAllProjectsForCards = async () => {
...
export const getAllSkills = async () => {
  const skills = await Skill.find().sort({ order: -1 }).select('title logo').lean();
...
export const getSkillCount = async () => Skill.countDocuments();
...
export const getBlogCount = async () => Blog.countDocuments({ status: 'published' });
export const getProjectCount = async () => Project.countDocuments();
export const getProjectCountByTech = async (techName: string) => {
  const regex = new RegExp(techName, 'i');
...
const buildPortfolioOverviewContext = async () => {
  const [skills, projects] = await Promise.all([getAllSkills(), getAllProjectsForCards()]);
  const skillCount = skills.length;
  const projectCount = projects.length;
  const aiSkills: string[] = [], frontendSkills: string[] = [], backendSkills: string[] = [], cloudSkills: string[] = [];
  ...
  skills.forEach((s) => {
    const techs = s.logo.filter((item: string) => !item.startsWith('http'));
  ...
  const experienceDetails = EXPERIENCE_DATA.map((exp) => `- **${exp.title}** at ${exp.company} (${exp.duration})\n  _${exp.description}_`).join('\n\n');
  const educationDetails = EDUCATION_DATA.map((edu) => `- **${edu.degree}**\n  ${edu.institution} | ${edu.duration}\n  _${edu.description}_`).join('\n\n');
...
const buildContextFromResults = (results: any[]) =>
  results.map((r) => {
...
// ==================== Chat Function ====================
export const chat = async (message: string, history: { role: string; content: string }[]) => {
  ...
  if (isGreeting && !detectedTech && !isSkillQuery && !isBlogQuery && !isPortfolioOverview &&
    ...
    if (lower.includes('thank')) greeting = "You're welcome! If you have any more questions about Subir's work, skills, or availability — I'm always here to help!";
    else if (lower.includes('bye') || lower.includes('goodbye')) greeting = "Goodbye! Thanks for visiting Subir's portfolio. Come back anytime!";
    else if (lower.includes('hey') || lower.includes('yo') || lower.includes('sup')) greeting = "Hey there! Welcome to Subir's portfolio. I'm his AI Assistant — ready to help you explore projects, skills, or anything you need. What's up?";
    return { success: true, message: greeting, status: 'SUCCESS' };
  }

  // Fixed Intent Responses
  if (isJobExperience && !isPortfolioOverview) return { success: true, message: buildJobExperienceResponse(SITE_URL), status: 'SUCCESS' };
  if (isExperienceDuration && !isPortfolioOverview) return { success: true, message: buildExperienceDurationResponse(SITE_URL), status: 'SUCCESS' };
  if (isFreelancingQuery && !isPortfolioOverview && !isBlogQuery && !isSkillQuery) return { success: true, message: buildFreelancingResponse(SITE_URL), status: 'SUCCESS' };
  if (isCapabilityQuery && !isPortfolioOverview && !isBlogQuery && !isSkillQuery && !isAIAutomationQuery && !isFreelancingQuery) {
    ...
    return { success: true, message: buildCapabilityResponse(what, SITE_URL), status: 'SUCCESS' };
  }
  ...
  if (isFollowUp && !detectedTech && !isSkillQuery && !isBlogQuery && !isPortfolioOverview) {
    ...
    if (prevTech) {
      ...
      if (projectCards.length > 0) {
        const techContext = projectCards.map((p) => `PROJECT: ${p.title}\nSlug: ${p.slug}\nDetail URL: ${PROJECT_DETAILS_URL(p.slug)}\nType: ${p.projectType}\nTechnologies: ${p.technologies.join(', ')}\nDescription: ${p.shortDescription}\nLive Link: ${p.liveLink}\nImage: ${p.imageUrls?.[0] || ''}`).join('\n---\n');
      ...
      const skillContext = skillCards.map((s) => `SKILL: ${s.title}\nTechnologies: ${s.logo.filter((i: string) => !i.startsWith('http')).join(', ')}`).join('\n---\n');
  ...
  if (isAllProjectsQuery && !context) {
    ...
    if (allProjects.length > 0) {
      const projectContext = allProjects.map((p) => `PROJECT: ${p.title}\nSlug: ${p.slug}\nDetail URL: ${PROJECT_DETAILS_URL(p.slug)}\nType: ${p.projectType}\nTechnologies: ${p.technologies.join(', ')}\nDescription: ${p.shortDescription}\nLive Link: ${p.liveLink}\nImage: ${p.imageUrls?.[0] || ''}`).join('\n---\n');
  ...
  if (!context) {
    ...
    if (results.length === 0 || results[0].score < SIMILARITY_THRESHOLD) {
      ...
      if (isAIAutomationQuery || lower.includes('automation') || lower.includes('agent') || lower.includes('bot')) {
        ...
        }).map((s) => `${s.title}: ${s.logo.filter((i: string) => !i.startsWith('http')).join(', ')}`).join('\n');
      ...
      if (!context && (lower.includes('freelanc') || lower.includes('hire') || lower.includes('available'))) {
        return { success: true, message: buildFreelancingResponse(SITE_URL), score: results[0]?.score || 0, status: 'FAILED' };
      }
      if (!context) {
        return { success: true, message: buildNegativeFallbackResponse(SITE_URL), score: results[0]?.score || 0, status: 'FAILED' };
      }
    } else {
      ...
      if (projectResults.length > 0) {
        ...
        const mongoProjects = await Project.find({ title: { $in: titles } })
          .select('title slug shortDescription liveLink imageUrls projectType technologies')
          .lean();
        projectCards = mongoProjects.map((p: any) => ({
          _id: p._id?.toString() || '',
        ...
        const projectDetails = projectCards.map((p) => `PROJECT: ${p.title}\nSlug: ${p.slug}\nURL: ${PROJECT_DETAILS_URL(p.slug)}\nImage: ${p.imageUrls?.[0] || ''}\nLive: ${p.liveLink}`).join('\n---\n');
  ...
  if (!response) {
    ...
    if (projectsToUse.length === 0) {
      try { projectsToUse = await getAllProjectsForCards(); } catch (_) {}
    }
    if (projectsToUse.length > 0) {
