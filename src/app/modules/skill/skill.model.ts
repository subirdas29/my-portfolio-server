...
SkillSchema.post('save', async function () {
  if (process.env.NODE_ENV !== 'test') {
    try {
      ...
      await AIServices.upsertSkillToAI({ _id: doc._id, title: doc.title, logo: doc.logo, order: doc.order });
    } catch (error) {
...
SkillSchema.post('findOneAndUpdate', async function () {
  if (process.env.NODE_ENV !== 'test') {
    try {
      ...
      if (doc) {
        const d = doc as any;
        await AIServices.upsertSkillToAI({ _id: d._id, title: d.title, logo: d.logo, order: d.order });
      }
    } catch (error) {
