...
const createSkillController = catchAsync(async (req, res) => {
  ...
  sendResponse(res, { statusCode: httpStatus.CREATED, success: true, message: 'Skill is created successfully', data: result });
});

const getAllSkill = catchAsync(async (req, res) => {
  const result = await SkillServices.getAllSkill(req.query);
  sendResponse(res, { statusCode: httpStatus.OK, success: true, message: 'Skills fetched successfully', meta: result.meta, data: result.result });
});

const updateSkillOrderController = catchAsync(async (req, res) => {
  const result = await SkillServices.updateSkillOrder(req.body);
  CacheUtils.clearCache(['/api/v1/skills']);
  sendResponse(res, { statusCode: httpStatus.OK, success: true, message: 'Skill order updated successfully', data: result });
});
